 ...
 * @see WP_Image_Editor
 */
class WP_Pronotron_Image_Editor_Imagick extends WP_Image_Editor {
	/**
	 * Imagick object.
	 *
	 * @var Imagick
	...
	public function rotate( $angle ) {
		return new WP_Error( 'Rotation is not supported.' );
	}

	public function flip( $horz, $vert ) {
		return new WP_Error( 'Flipping is not supported.' );
	}
	...
	public function make_subsize( $size_data ){
		...
		if ( ! isset( $size_data['width'] ) && ! isset( $size_data['height'] ) ){
			return new WP_Error( 
				...
				__( 'Cannot resize the image. Both width and height are not set.' ) 
			);
		}
		...
		/**
		 * @edited
		 * - That part is removed, avoids full size cropping
		 * - That part also avoids same size cropping with source while new upload, 
		 *   so if any custom subsize is missing, original image is the size
		 * 
		 */
